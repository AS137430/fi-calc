  ...
  const stop = springAnimation({
    ...
    onUpdate(update) {
      ...
      if (
        ...
        if (typeof onChangeDestination === 'function') {
          onChangeDestination(newDestinationPoint);
        }
...
export default function useTouchMotion({
  el,
  active = true,
  points = [],
  movement = {},
  ...
  function onTouchEndEvent(e) {
    ...
    if (initialY !== 0 || initialX !== 0) {
      ...
      console.log('comparing', pointsRef.current, destinationPoint);
