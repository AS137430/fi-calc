export const morph = time => ({
  componentWillEnter(cb, overEl, triggerEl) {
    ...
    requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        overEl.style.transition = `opacity ${time}ms cubic-bezier(0.175, 0.885, 0.32, 1.08), transform ${time}ms cubic-bezier(0.175, 0.885, 0.32, 1.08)`;
  ...
  componentWillLeave(cb, overEl, triggerEl) {
    const bb = triggerEl.getBoundingClientRect();
    const mybb = overEl.getBoundingClientRect();
    overEl.style.transition = `opacity ${time}ms cubic-bezier(0.6, 0, 0.735, 0.045), transform ${time}ms cubic-bezier(0.6, 0, 0.735, 0.045)`;
